...
#define PLAYER_PLAYING (character_generated && p_ptr && \
	p_ptr->state.playing && !p_ptr->state.is_dead)
...
void init_android_stuff(void)
{
	...
	my_strcpy(player_name, android_savefile, sizeof(player_name));
}
...
void angdroid_main()
{
	...
	/* Initialize dirs and other stuff */
	//create_needed_dirs();
