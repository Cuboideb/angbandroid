...
MINDBLAST2 = add_spell
{
	...
	["fail"] = 	45,
	["spell"] = 	function()
			if get_level(MINDBLAST2, 50) >= 20 then
				local ret, dir = get_aim_dir()
				if ret == FALSE then return end
				return fire_ball(GF_PSI, dir, 10 + get_level(MINDBLAST2, 150), 3)
			else
				local ret, dir = get_aim_dir()
				if ret == FALSE then return end
				return fire_bolt(GF_PSI, dir, 10 + get_level(MINDBLAST2, 150))
			end
	end,
	["info"] = 	function()
