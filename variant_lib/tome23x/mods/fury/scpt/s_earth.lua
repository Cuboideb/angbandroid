...
SHARDSTORM = add_spell
{
	...
	["spell"] = 	function()
			...
			if ret == FALSE then return end
			if get_level(STRIKE, 50) >= 12 then
				return fire_ball(GF_SHARDS, dir, 50 + get_level(SHARDSTORM, 50), 4)
			else
				return fire_ball(GF_SHARDS, dir, 50 + get_level(SHARDSTORM, 50), 2)
			end
	end,
	["info"] = 	function()
			if get_level(STRIKE, 50) >= 12 then
	       			return "dam "..(50 + get_level(SHARDSTORM, 50)).." rad 1"
			else
				return "dam "..(50 + get_level(SHARDSTORM, 50))
			end
	end,
	["desc"] =	{
			"Creates a ball of shards",
			"At level 12 it turns into a ball of radius 4"
	}
}
